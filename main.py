...
univs_db = CsvTable('output/universities.csv', ['id', 'title', 'link', 'city', 'city_id', 'learning_cost', 'budget_places', 'paid_places'])
progs_db = CsvTable('output/programs.csv', ['id', 'title', 'link', 'learning_cost', 'budget_places', 'paid_places'])
subs_db = CsvTable('output/subjects.csv', ['id', 'title'])

univs_progs_db = CsvTable('output/univs-progs.csv', ['university-id', 'program-id'])
progs_subs_db = CsvTable('output/progs-subs.csv', ['program-id', 'subject-id'])
...
def main():
    for spec_code in spec_codes:
        ...
        for page in range(1, 21):
            ...
            current_page, univs = postupi.univs().get_catalog(spec_code, page)
